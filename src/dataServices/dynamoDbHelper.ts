...
export default class DynamoDbHelper {
    ...
    async getMostRecentResource(
        ...
        const params = DynamoDbParamBuilder.buildGetResourcesQueryParam(id, 1, '', projectionExpression); // TODO add tenantID support for bundle requests
    ...
    async getMostRecentValidResource(resourceType: string, id: string, tenantId: string): Promise<GenericResponse> {
        const params = DynamoDbParamBuilder.buildGetResourcesQueryParam(id, 2, tenantId);
        let item = null;
        const result = await this.dynamoDb.query(params).promise();
